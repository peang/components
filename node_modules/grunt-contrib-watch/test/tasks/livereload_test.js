...
exports.livereload = {
  ...
  onlytriggeron: function(test) {
    ...
    assertWatch([function() {
      request(35729, function(data) {
        grunt.file.write(path.join(cwd, 'sass', 'one.sass'), '#one {}');
      });
    }], function(result) {
      result = helper.unixify(result);
      helper.verboseLog(result);
      test.ok(result.indexOf('Live reloading sass/one.sass') === -1, 'Should not trigger live reload on non livereload targets.');
